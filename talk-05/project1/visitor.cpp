...
class DrawPrimitive {
public:
    virtual void save_to(class Export *) = 0;
};
...
public:
    ...
    void save_to(class Export *) override;
};
...
void Circle::save_to(Export *v) {
    v->save_to(this);
}

void Box::save_to(Export *v) {
    v->save_to(this);
}
...
int main(int, char *[]) {
    ...
    JsonExport json_exporter;
    for (auto obj : doc) {
        obj->save_to(&json_exporter);
    }
