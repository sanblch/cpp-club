...
// sample(dsl)
template <typename Signature> constexpr hana::basic_type<Signature> function{};

template <char... c> struct event {
  template <typename F> constexpr auto operator=(F f) const {
    return hana::make_pair(*this, f);
  }
};

// template <typename CharT, CharT... c> constexpr event<c...> operator""_e() {
//   return {};
// }
// end-sample
...
namespace boost {
namespace hana {
template <char... c> struct tag_of<::event<c...>> { using type = ::event_tag; };

template <> struct equal_impl<::event_tag, ::event_tag> {
  template <typename X, typename Y> static constexpr auto apply(X, Y) {
    return std::is_same<X, Y>{};
  }
};

template <> struct hash_impl<::event_tag> {
  template <typename Event> static constexpr auto apply(Event const &) {
